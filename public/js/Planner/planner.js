...
var modelSystemPlanner = function(_initialTasks, _modelParts, _workers){
...
var createTasks = function(rId, tasks){
	var td = $('<td></td>');
	var queue = $('<div class="model-system-task-queue"></div>');
	for(var j=0;j<tasks.length;j++){
...
var chooseWorker = function(e){
	...
	var deptSelect = $('<select></select>');
	deptSelect.append('<option value="'+0+'">'+'请选择'+'</option>');
	for(var i=0;i<workers.length;i++){
		deptSelect.append('<option value="'+(i+1)+'">'+workers[i][0]+'</option>')
	}

	var personContainer = $('<div class="mask-part-container"></div>');
	var personLabel = $('<span>工人姓名</span>');
	var personSelect = $('<select></select>');
	...
	deptSelect.bind('change', function(){
		var val = deptSelect.val()-1;
		personSelect.empty();
		if(val<0)
			return;
		personSelect.append('<option value="'+0+'">'+'请选择'+'</option>');
		for(var i=0;i<workers[val][1].length;i++){
			var workerId = workers[val][1][i][0];
			var workerName = workers[val][1][i][1];
			personSelect.append('<option workerId="'+workerId+'" workerName="'+workerName+'" value="'+(i+1)+'">'+workerName+'('+workerId+')'+'</option>')
		}
	});
	var data = {};
	data['target'] = target;
...
var publishTask = function(e){
	...
	if(!personSelect.val() || personSelect.val() == 0){
		message.html('未选择工人');
		return;
	}
	var p = target.find('p');
	...
	var workerId = personSelect.find("option:selected").attr('workerid');
	...
	if(confirm(confirmStr)){
		$.ajax({
			...
			type:'POST',
			data:{'modelId': modelId,'partId': partId,'procedureName': procedureName,'workerId': workerId,'timeStamp': timeStamp},
			success: function(data){
				data = data.trim();
				if(data == 'success'){
...
var chooseAndAdd = function(target, position, hasDate){
	...
	var modelSelect = $('<select></select>');
	modelSelect.append('<option value="'+0+'">'+'请选择'+'</option>');
	for(var i=0;i<modelParts.length;i++){
		modelSelect.append('<option modelId="'+modelParts[i][0][0]+'" value="'+(i+1)+'">'+modelParts[i][0][1]+'</option>')
	}

	
	var partContainer = $('<div class="mask-part-container"></div>');
	var partLabel = $('<span>零件编号</span>');
	var partSelect = $('<select></select>');

	var procedureContainer = $('<div class="mask-part-container"></div>');
	var procedureLabel = $('<span>工序名</span>');
	var procedureSelect = $('<select></select>');
	procedureSelect.append('<option value="'+0+'">'+'请选择'+'</option>');
	for(var i=0;i<optionalProcedure.length;i++){
		procedureSelect.append('<option value="'+(i+1)+'">'+optionalProcedure[i]+'</option>')
	}
	...
	modelSelect.bind('change', function(){
		var val = modelSelect.val()-1;
		partSelect.empty();
		if(val<0)
			return;
		partSelect.append('<option value="'+0+'">'+'请选择'+'</option>');
		for(var i=0;i<modelParts[val][1].length;i++){
			var partId = modelParts[val][1][i][0];
			var partCode = modelParts[val][1][i][1];
			var partName = modelParts[val][1][i][2];
			partSelect.append('<option partid="'+partId+'" partcode="'+partCode+'" partname="'+partName+'" value="'+(i+1)+'">'+partCode+'('+partName+')'+'</option>')
		}
	});
}
...
var addTask = function(e){
	...
	if(modelSelect.val()==0 || partSelect.val()==0 || procedureSelect.val()==0){
